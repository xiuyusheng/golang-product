...
func GenRsaKey() (PrivateKey, PublicKey string) {
	priKey, err2 := rsa.GenerateKey(rand.Reader, conf.Config.Rsa.Rsa_bit)
	if err2 != nil {
		panic(err2)
	}
...
func init() {
	// 设置定时器，每隔一段时间执行一次任务
	interval := time.Duration(conf.Config.Rsa.Timing) * time.Hour // 每隔24小时执行一次任务
